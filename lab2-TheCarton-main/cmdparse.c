...
cmd_free(command_t *cmd)
{
    ...
    for (j = 0; j < 3; j++) {
        if (cmd->redirect_filename[j] != 0) {
            free(cmd->redirect_filename[j]);
        }
    }
...
cmd_parse(parsestate_t *parsestate)
{
	...
	while (1) {
		...
		case TOK_OPEN_PAREN: {
            ...
            // i want a linked list of commands for the subshell

            // parenthesis means we need output / input destination like
            // end command or >.

            // create a command state with command_t parens set to true
            // call cmd_line_parse:
            // cmd_line_parse(parsestate_t *parsestate, int in_parens)
            // this recursive call will give us a new command_t that has the entire subshell command
            // so we set the top-level shell subshell pointer to the linked list from the recursive call
            // we have to handle errors with open paren, cmd_line_parse will handle close paren errors
            // token_t next_token;
            // parse_gettoken(parsestate, &next_token);
            command_t *subcommand = cmd_line_parse(parsestate, 1);
            cmd->subshell = subcommand;
            break;
        }
		default:
			parse_ungettoken(parsestate);
    ...
    // some problem with subshell here
